...
const StatCard: React.FC<StatCardProps> = ({ label, value, icon, color }) => {
  ...
  return (
    <div className="bg-white p-4 sm:p-6 rounded-2xl border border-slate-200 flex flex-col gap-2 shadow-sm transition-all hover:shadow-md hover:-translate-y-1">
      <div
        className={`w-10 h-10 rounded-xl flex items-center justify-center ${color !== 'gold' ? colors[color] : ''}`}
        style={color === 'gold' ? { background: 'var(--gold-dim)', color: 'var(--gold)' } : {}}
      >
        {icon}
      </div>
      <div>
        <div className="font-display text-2xl sm:text-3xl font-bold text-slate-900 tracking-tight">{value}</div>
